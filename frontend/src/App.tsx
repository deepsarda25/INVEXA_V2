...
export default function App() {
  ...
  const oneDayPnlQuery = useQuery({
    queryKey: ["one-day-pnl", holdingsList.map((h) => h.ticker).join(",")],
    queryFn: async () => {
      const entries = await Promise.all(
        holdingsList.map(async (h) => {
          try {
            const profile = await apiFetch<{ previousClose: number | null }>(`/stocks/${h.ticker}/profile`);
            return { ticker: h.ticker, previousClose: profile.previousClose };
          } catch {
            return { ticker: h.ticker, previousClose: null };
          }
        })
      );
      let total = 0;
      for (const h of holdingsList) {
        const match = entries.find((e) => e.ticker === h.ticker);
        const prevClose = match?.previousClose;
        const livePrice = prices[h.ticker]?.price ?? h.livePrice;
        if (prevClose) total += (livePrice - prevClose) * h.quantity;
      }
      return total;
    },
    enabled: holdingsList.length > 0,
    staleTime: 60_000
  });
  const oneDayPnl = oneDayPnlQuery.data ?? 0;
          ...
          {activeTab === "market" && (
            <>
              ...
              <div className="stats-row" style={{ gridTemplateColumns: "repeat(5, minmax(0, 1fr))" }}>
                ...
                <div className="stat-cell">
                  <span>1D Profit</span>
                  <strong className={oneDayPnl >= 0 ? 'good headline-sm' : 'bad headline-sm'}>
                    {oneDayPnl >= 0 ? '+' : ''}
                    ₹{oneDayPnl.toLocaleString("en-IN", { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                  </strong>
                </div>
                <div className="stat-cell">
