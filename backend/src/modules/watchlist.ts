...
export const watchlistModule = new Elysia({ prefix: "/watchlist" })
  ...
  .get("/lists/:listId", async (ctx: any) => {
    ...
    const items = await Promise.all(
      rows.map(async (row) => {
        const [livePrice, profile] = await Promise.all([getLivePrice(row.ticker), getCachedProfile(row.ticker)]);
        ...
        const previousClose = profile?.previousClose ?? null;
        const changeAbs = typeof livePrice === "number" && previousClose ? livePrice - previousClose : null;
